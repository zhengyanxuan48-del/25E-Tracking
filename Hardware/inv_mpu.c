...
volatile int MPU6050_I2C_LastStatus = 0;
volatile uint32_t MPU6050_I2C_LastError = 0;
volatile uint8_t MPU6050_I2C_LastOp = 0;
volatile uint8_t MPU6050_I2C_LastDev = 0;
volatile uint8_t MPU6050_I2C_LastReg = 0;
volatile uint16_t MPU6050_I2C_LastSize = 0;

static int i2c_write(unsigned char dev_addr, unsigned char reg_addr,
                     unsigned short data_size, const unsigned char *p_data)
{
    platform_i2c_status_t status;

    status = platform_i2c_write_reg(dev_addr, reg_addr, p_data, data_size);

    MPU6050_I2C_LastStatus = status;
    MPU6050_I2C_LastError = (status == PLATFORM_I2C_OK) ? 0U : (uint32_t) -status;
    MPU6050_I2C_LastOp = 'W';
    MPU6050_I2C_LastDev = dev_addr;
    MPU6050_I2C_LastReg = reg_addr;
    MPU6050_I2C_LastSize = data_size;

    return status;
}

static int i2c_read(unsigned char dev_addr, unsigned char reg_addr,
                    unsigned short data_size, unsigned char *p_data)
{
    platform_i2c_status_t status;

    status = platform_i2c_read_reg(dev_addr, reg_addr, p_data, data_size);

    MPU6050_I2C_LastStatus = status;
    MPU6050_I2C_LastError = (status == PLATFORM_I2C_OK) ? 0U : (uint32_t) -status;
    MPU6050_I2C_LastOp = 'R';
    MPU6050_I2C_LastDev = dev_addr;
    MPU6050_I2C_LastReg = reg_addr;
    MPU6050_I2C_LastSize = data_size;
...
{
    platform_delay_ms((uint32_t) ms);
}

static unsigned long mpu_get_ms(void)
{
    return (unsigned long) platform_time_ms();
}

#define delay_ms mpu_delay_ms
#define get_ms(p) do{ *(p) = mpu_get_ms();}while(0)
...
#if defined MPU6050
    /*
     * ���� MPU6050 / GY-521 ģ��� PROD_ID ���ܶ��� 0��
     * ԭ�� InvenSense ��������� return -1��
     * ��ֻҪǰ�� WHO_AM_I �Ѿ��ܶ��� 0x68��������������ʼ����
     */
