...
static void dmp_delay_ms(unsigned long ms) __attribute__((unused));
static void dmp_delay_ms(unsigned long ms)
{
    platform_delay_ms((uint32_t) ms);
}

static unsigned long dmp_get_ms(void)
{
    return (unsigned long) platform_time_ms();
}

#define delay_ms dmp_delay_ms
#define get_ms(p) do{ *(p) = dmp_get_ms();}while(0)
